...
function getDateAgo(date, days) {
  ...
  dateInfo.setDate(dateInfo.getDate() - days);

  return dateInfo.toLocaleString().replace(/(\d.*),\s+(\d.*)/gu, '$1');
...
Object.defineProperties(Car.prototype, {
  used: {
    get() {
      ...
      return year - this.year > 1 ? 'used' : 'new';
    },
    set(value) {
      const year = new Date().getFullYear();

      if (value === 'new' && this.year < year) this.year = year;
    }
  }
});
